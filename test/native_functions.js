...
Vows.describe('evaluate simple instructions').addBatch({
    ...
    'test plus' : function () {
      var val1 = Functions.plus([1, 2, 3, 4, 5]);
      Assert.strictEqual(val1, 15);

      var val2 = Functions.plus([1, 2, 3, 4, -5]);
      Assert.strictEqual(val2, 5);

      Assert.throws(
        function () {
          var val3 = Functions.plus([]);
        },
      ...
      Assert.throws(
        function () {
          var val4 = Functions.plus([1, 'hurr']);
        },
        /is not a number/
      );
    }, 
    ...
    'test minus' : function () {
      var val1 = Functions.minus([1, 2, 3, 4, 5]);
      Assert.strictEqual(val1, -13);

      var val2 = Functions.minus([1, 2, 3, 4, -5]);
      Assert.strictEqual(val2, -3);

      Assert.throws(
        function () {
          var val3 = Functions.minus([]);
        },
        /At least two arguments are needed./
      );
    },
    ...
    'test multiply' : function () {
      var val1 = Functions.multiply([1, 2, 3, 4, 5]);
      Assert.strictEqual(val1, 120);

      var val2 = Functions.multiply([1, 2, 3, 4, -5]);
      Assert.strictEqual(val2, -120);
    },

    'test divide' : function () {
      var val1 = Functions.divide([1, 2, 3, 4, 5]);
      Assert.strictEqual(val1, 0.008333333333333333);

      var val2 = Functions.divide([1, 2, 3, 4, -5]);
      Assert.strictEqual(val2, -0.008333333333333333);

      var val4 = Functions.divide([4, 0]);
